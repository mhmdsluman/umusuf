...
import { showToast, showLoader, renderIcons, showConfirm } from './utils.js';
        ...
        <div class="bg-white dark:bg-gray-800 p-2 rounded-lg shadow-md">
            ...
            <div class="p-6">
                ...
                <div id="financials-settings-tab" class="settings-tab-content hidden space-y-4">
                    ...
                    <div>
                        <label class="block mb-1 font-semibold">آخر يوم لدفع الرسوم</label>
                        <input type="number" id="setting-feeDueDate" value="${settings.feeDueDate}" min="1" max="28" class="w-full p-2 border rounded dark:bg-gray-700">
                    </div>
                </div>
...
async function saveAllSettings() {
    ...
    try {
        ...
        const newSettings = {
            ...
            feeDueDate: parseInt(document.getElementById('setting-feeDueDate').value) || 5,
            passingScore: parseInt(document.getElementById('setting-passingScore').value) || 50,
            examFields: examFields,
        };

        await db.settings.put(newSettings);
        addNotification('تم تحديث الإعدادات بنجاح.', 'admin');
...
async function backupData() {
    ...
    try {
        ...
        a.download = `tahfeez-backup-${date}.json`;
...
function handleRestoreFile(event) {
    ...
    showConfirm('هل أنت متأكد من رغبتك في استعادة البيانات من هذا الملف؟ سيتم حذف جميع البيانات الحالية.', () => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                showToast('فشل استعادة البيانات. تأكد من أن الملف صحيح.', 'error');
            } finally {
...
async function restoreData(data) {
    ...
    try {
        ...
        addNotification('تم استعادة البيانات بنجاح! سيتم إعادة تحميل التطبيق.', 'admin');
        setTimeout(() => location.reload(), 2000);
    } finally {
        // The loader will be hidden by the page reload
    }
}

async function clearLocalData() {
    showConfirm('هل أنت متأكد تماماً من رغبتك في حذف جميع البيانات المحلية؟ سيتم فقدان كل شيء بشكل دائم.', async () => {
        showLoader(true);
        try {
            await db.delete();
            showToast('تم مسح جميع البيانات المحلية بنجاح. سيتم إعادة تحميل التطبيق.', 'success');
            setTimeout(() => location.reload(), 2000);
        } catch (error) {
