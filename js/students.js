...
function renderClassHeader(classId) {
    ...
    if (selectedClass) {
        let groupLinks = '';
        if (selectedClass.whatsapp_link) {
            groupLinks += `<a href="${selectedClass.whatsapp_link}" target="_blank" class="bg-green-500 text-white px-3 py-1 rounded-lg text-sm flex items-center gap-1"><i data-lucide="message-circle" class="w-4 h-4"></i> WhatsApp</a>`;
        }
        if (selectedClass.telegram_link) {
            groupLinks += `<a href="${selectedClass.telegram_link}" target="_blank" class="bg-sky-500 text-white px-3 py-1 rounded-lg text-sm flex items-center gap-1"><i data-lucide="send" class="w-4 h-4"></i> Telegram</a>`;
        }
        container.innerHTML = `
            <div class="flex flex-wrap justify-between items-center mb-4 bg-gray-100 dark:bg-gray-800 p-4 rounded-lg">
                <h3 class="text-xl font-semibold">طلاب حلقة: ${selectedClass.name}</h3>
                <div class="flex gap-2">${groupLinks}</div>
            </div>
        `;
        renderIcons(); // Re-render icons for the new buttons
    } else {
        container.innerHTML = '';
    }
}
...
export async function initStudents(container) {
    ...
    container.innerHTML = `
        ...
        <div id="class-header-container"></div>
    ...
    if (classIdFromUrl) {
        classFilterSelect.value = classIdFromUrl;
    }

    // Initial render of header
    renderClassHeader(classFilterSelect.value);
...
async function renderStudentsTable() {
    ...
    if (students.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="6" class="text-center p-4 text-gray-500">لا يوجد طلاب لعرضهم.</td></tr>`;
        return;
    }
...
async function showStudentModal(studentId = null) {
    ...
    showModal(title, formContent, async () => {
        ...
        // Automatically set the first month's payment as pending
        const currentMonth = new Date().toISOString().slice(0, 7);
        await db.financials.put({ student_id: studentData.id, month_year: currentMonth, status: 'pending' });
        
        addNotification(`تم تسجيل طالب جديد: ${studentData.name}`, 'admin');
        hideModal();
        await renderStudentsTable();
    });
}
...
async function deleteStudent(studentId) {
    showConfirm('هل أنت متأكد من رغبتك في حذف هذا الطالب؟ سيتم حذف جميع سجلاته المرتبطة.', async () => {
        try {
            ...
            addNotification(`تم حذف الطالب: ${student.name}`, 'admin');
            await renderStudentsTable();
        } catch (error) {
