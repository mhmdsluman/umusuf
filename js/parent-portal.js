...
async function renderMemorizationProgressForParent(student) {
    // This is a simplified view for the parent portal
    const QURAN_JUZ_COUNT = 30;
    let gridHtml = '<div class="space-y-3">';
    for (let i = 1; i <= QURAN_JUZ_COUNT; i++) {
        // In a real app, you'd calculate pages memorized per juz
        const pagesMemorized = Math.floor(Math.random() * 21); // Placeholder
        const percentage = (pagesMemorized / 20) * 100;
        gridHtml += `
            <div>
                <div class="flex justify-between text-sm mb-1">
                    <span>الجزء ${i}</span>
                    <span>${pagesMemorized}/20</span>
                </div>
