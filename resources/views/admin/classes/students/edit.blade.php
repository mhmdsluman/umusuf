<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Enroll Students in Class: ') }} {{ $class->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.classes.students.update', $class) }}">
                        @csrf
                        @method('PUT')

                        <div class="space-y-4">
                            <label class="block font-medium text-sm text-gray-700">{{ __('Select Students to Enroll') }}</label>
